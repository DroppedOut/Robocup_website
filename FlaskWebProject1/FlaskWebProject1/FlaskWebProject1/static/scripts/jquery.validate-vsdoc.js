				...
				function handle() {
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							var hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.name).val(validator.submitButton.value).applicationendTo(validator.currentForm);
						}
...
$.validator.format = function(source, params) {
	...
	if ( arguments.length == 1 ) 
		return function() {
			...
			args.unshift(source);
			return $.validator.format.applicationly( this, args );
		};
...
$.extend($.validator, {
	...
	defaults: {
		...
		onfocusin: function(element) {
			...
			if ( this.settings.focusCleanup && !this.blockFocusCleanup ) {
				...
				this.addWrapplicationer(this.errorsFor(element)).hide();
			}
		},
	...
	prototype: {
		...
		hideErrors: function() {
			this.addWrapplicationer( this.toHide ).hide();
		},
		...
		addWrapplicationer: function(toToggle) {
			if ( this.settings.wrapplicationer )
				toToggle = toToggle.add( toToggle.parent( this.settings.wrapplicationer ) );
			return toToggle;
		},
		...
		defaultShowErrors: function() {
			...
			this.hideErrors();
			this.addWrapplicationer( this.toShow ).show();
		},
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				if ( this.settings.wrapplicationer ) {
					...
					// actually showing the wrapplicationed element is handled elsewhere
					label = label.hide().show().wrap("<" + this.settings.wrapplicationer + "/>").parent();
				}
				if ( !this.labelContainer.applicationend(label).length )
	...
	if ( $.ajaxPrefilter ) {
		...
		$.ajax = function(settings) {
			...
			if (mode == "abort") {
			    ...
			    return (pendingRequests[port] = ajax.applicationly(this, arguments));
		    }
		    return ajax.applicationly(this, arguments);
	    };
    }
})(jQuery);
		...
		$.each({
			...
			$.event.special[fix] = {
				...
				handler: function(e) {
					...
					return $.event.handle.applicationly(this, arguments);
				}
			};
			function handler(e) {
				e = $.event.fix(e);
		...
		validateDelegate: function(delegate, type, handler) {
			return this.bind(type, function(event) {
				...
				if (target.is(delegate)) {
					return handler.applicationly(target, arguments);
				}
			});
		}
	});
